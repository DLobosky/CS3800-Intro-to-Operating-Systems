...
void *sub2(void *t) 
{
  ...
  for (i=0; i<ITERATIONS; i++) {
    ...
    */
    if (count == THRESHOLD) {
//    pthread_cond_wait(&count_condvar, &count_mutex);  // just trying to get it to wait for signal
...
int main(int argc, char *argv[])
{
  ...
  pthread_attr_init(&attr);   // Don't worry about
  pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);   // Don't worry about
